...
export default function ApplicationForm({ showAdmissionLetter = false, applicationStatus }: ApplicationFormProps) {
  ...
  const handlePhoto = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_FILE_SIZE) {
      alert("The image is too large. Please select a passport photo smaller than 5MB.");
      e.target.value = ""; // Clear the input so the same file can't be submitted
      return;
    }

    // If size is okay, proceed with preview and state update
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await axios.post("https://jayone-87f0a69e6159.herokuapp.com/api/applications/submit", form);
      // const res = await axios.post("http://localhost:5000/api/applications/submit", form);
                    ...
                    <div className={styles.photoUpload}>
                      ...
                      <label className={styles.photoBox}>
                        ...
                        <label className={styles.label}>
                          Passport Photo <span style={{ fontSize: '10px', color: '#666' }}>(Max 5MB)</span>
                        </label>
