...
// Helper for download application (reuse logic from ApplicationForm)
...
function handleDownloadAdmissionLetter(status: any, formData: any) {
  ...
  y += 8;
  doc.text('We are pleased to inform you that you have been granted admission to Jayone Prestige School of Fashion.', 10, y, { maxWidth: 190 });
  y += 12;
  doc.setFont('helvetica', 'bold');
  ...
  const years = status?.years || (course === '2years' ? '2 Years' : course === '1year' ? '1 Year' : course === '6months' ? '6 Months' : course === '3months' ? '3 Months' : '');
  ...
  y += 8;
  doc.setFont('helvetica', 'normal');
  doc.text('Jayone Prestige School of Fashion is dedicated to nurturing creative talent and providing hands-on training in fashion and design. Our programs are tailored to equip you with the skills needed for a successful career in the fashion industry.', 10, y, { maxWidth: 190 });
  y += 18;
  doc.setFont('helvetica', 'bold');
  doc.text('Next Steps:', 10, y);
  y += 8;
  doc.setFont('helvetica', 'normal');
  doc.text('Please contact the school administration to receive your full prospectus and for any further information regarding your admission and enrollment process.', 10, y, { maxWidth: 190 });
  y += 18;
...
export default function SuccessPage() {
  ...
  useEffect(() => {
    // Get form data from localStorage or API
    ...
    // Get status from API if token exists
    ...
    if (token) {
      fetch(`https://jayone-87f0a69e6159.herokuapp.com/api/status/${token}`)
        ...
        .then(data => {
          ...
          if (data && data.admissionGranted) setShowAdmissionLetter(true);
        });
    }
  }, []);

  if (!formData) return <div style={{textAlign:'center',marginTop:60}}>Loading...</div>;
